...
const targetProductDashboardIndex = 2;
const nameSuffix = 'X';
...
describe('Tutorial part 6', () => {
  ...
  function getPageElts() {
    ...
    return {
      ...
      heroDetail: element(by.css('app-root app-product-detail > div')),
  ...
  describe('Products tests', () => {
    ...
    it('can route to product details', async () => {
      await getProductLiEltById(targetProduct.id).click();

      const page = getPageElts();
      expect(await page.heroDetail.isPresent()).toBeTruthy('shows product detail');
      const product = await Product.fromDetail(page.heroDetail);
    ...
    it(`deletes ${newProductName} from Products list`, async () => {
      ...
      const page = getPageElts();
      expect(await page.appProducts.isPresent()).toBeTruthy();
      expect(await page.allProducts.count()).toEqual(8, 'number of products');
  ...
  describe('Progressive product search', () => {
    ...
    it(`continues search with 'n' and gets ${targetProduct.name}`, async () => {
    ...
    it(`navigates to ${targetProduct.name} details view`, async () => {
      ...
      const page = getPageElts();
      expect(await page.heroDetail.isPresent()).toBeTruthy('shows product detail');
      const hero2 = await Product.fromDetail(page.heroDetail);
      expect(hero2.id).toEqual(targetProduct.id);
      expect(hero2.name).toEqual(targetProduct.name.toUpperCase());
    });
  });
